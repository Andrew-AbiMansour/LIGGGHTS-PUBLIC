...
COHESION_MODEL(COHESION_JKR,jkr,3)
#else
...
namespace ContactModels {
  ...
  template<>
  class CohesionModel<COHESION_JKR> : protected Pointers {
  public:
    ...
    void connectToProperties(PropertyRegistry & registry) {
      ...
      registry.connect("cohEnergyDens", cohEnergyDens,"cohesion_model jkr");

      // error checks on coarsegraining
      if(force->cg_active())
        error->cg(FLERR,"cohesion model jkr");
    }
